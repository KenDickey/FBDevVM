...
{
  printf("KEY %3d %02x %c %s mod %02x\n",
	  key, key, ((key > 32) && (key < 127)) ? key : ' ',
	  up ? "UP" : "DOWN", modifiers);
/*xxxxxx
  modifierState= getModifiers();
  if (up)
    {
...
static void enqueueMouseEvent(int b, int dx, int dy)
{
  ...
  if (b)
    printf("mouse %02x at %4d,%4d mod %02x\n",
...
static int display_parseArgument(int argc, char **argv)
{
  ...
  if      (!strcmp(arg, "-vtlock"))	 vtLock=   1;
  ...
  else if (argv[1])	/* option requires an argument */
    {
      ...
      else if (!strcmp(arg, "-kbdev"))	 kbDev=   argv[1];
...
{
  if (key != 0) {
    printf("(%d = 0x%x) %s ", key, key, codename(EV_KEY, key) );
  }
}
...
{
  if (modifiers & ShiftKeyBit)   printf(" Shift");
  if (modifiers & CtrlKeyBit)    printf(" Control");
  if (modifiers & CommandKeyBit) printf(" Command");
  if (modifiers & OptionKeyBit)  printf(" Option");
}
...
static void recordKeyboardEvent(int keyCode, int pressCode, int modifiers, int ucs4)
{
  ...
  printf("EVENT (recordKeyboardEvent): time: %ld key", evt->timeStamp);
  switch (pressCode)
    {
